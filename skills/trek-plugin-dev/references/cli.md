...
| `preflight [dir] --repo <o/n> --tag <vX.Y.Z> [--all] [--entry <file.json>] [--zip] [--commit] [--sign]` | network | Runs the **full registry CI locally**: tag→commit, manifest parity (incl. `trek`/`operatorEgress`/`requiredAddons`/`pluginDependencies`), the `trek`-range gate (present, satisfiable, and matching the manifest), artifact sha256 + size, **author-signature shape + verification**, native scan, README gate. **Default checks only the newest version; `--all` checks every `versions[]`.** Green preflight ⇒ green CI **except** for CI-only gates: full JSON-schema validation (e.g. the 200-char description cap), homoglyph names, owner binding, and the **signing-downgrade guard** (it compares against the entry on the PR base, which only exists in CI). |
