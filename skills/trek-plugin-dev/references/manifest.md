...
| `capabilities.notificationChannel` | object | no | Declares an `integration` as a **notification delivery channel** (`{ events: [...] }`, validated against the channel-event list, e.g. `trip_invite`, `booking_change`, `trip_reminder` — narrow-only). Pairs with the **`hook:notification-channel`** grant and a `notificationChannel` hook implementation; `create` ships a `notification-channel` template. See [server-api.md](server-api.md). |
