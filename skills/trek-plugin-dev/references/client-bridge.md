...
| `trek:geolocation`     | `{ requestId, action?: 'get'\|'watch'\|'clear' }`                                                                             | Ask the HOST for the browser's live position — **needs the `geolocation:read` permission** (else `trek:error` code `forbidden`). The host reads `navigator.geolocation` (the browser's own site prompt still applies) and posts plain data back: `get` answers once (`trek:geolocation:result` with `position`), `watch` streams `trek:geolocation:update` fixes until `clear` (one watch per frame; the host force-stops it when the frame closes; the grant is re-checked on every fix). Nothing is sent to the server                                                                          |
| `trek:session:get`     | `{ requestId, key, scope?: 'plugin'\|'trip' }`                                                                                | Read host-managed **per-browser-tab** JSON state; answered as `trek:response` (`data` is `undefined` when unset). `scope:'plugin'` (default) is per plugin; `scope:'trip'` additionally partitions by the trip in view and fails with `NO_TRIP_CONTEXT` outside one                                                                                                                                                                                                                                                                                                                              |
| `trek:session:set`     | `{ requestId, key, value, scope? }`                                                                                           | Store a JSON-serialisable value. Quotas per scope: **32 keys, 64-char keys, 1 KiB per serialized value** — over → `trek:error` `SESSION_KEY_LIMIT` / `SESSION_INVALID_KEY` / `SESSION_VALUE_TOO_LARGE` (plus `SESSION_INVALID_VALUE` for unserialisable, `SESSION_STORAGE_ERROR` when the browser refuses). The storage key is host-owned and includes the signed-in user + your plugin id, so scopes never collide. **Not for secrets**                                                                                                                                                          |
...
| `trek:geolocation:result` | `{ requestId, position? \| watching? \| cleared? \| error? }` — the answer to a `trek:geolocation` request; `position` = `{ lat, lng, accuracy, heading, speed, timestamp }`, `error` ∈ `forbidden`/`denied`/`timeout`/`unavailable`/`unsupported`                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                            |
| `trek:geolocation:update` | `{ position?, error? }` — a streamed watch fix                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                |
...
> helpers wrap them: `trek.openExternal(url)`, `trek.confirm(opts)`
> (→ `Promise<boolean>`), `trek.onEvent(cb)`,
> `trek.session.get/set/remove/clear(…, { scope })`, and
> `trek.geolocation.get()` / `.watch(cb)` (→ unsubscribe fn; needs
> `geolocation:read`).
...
## On a phone — `viewport` (surface, fill, insets)

TREK's phone experience is its own design, and your plugin mounts inside it.
`trek:context.viewport` tells you where you are and how to size:

```js
m.viewport = {
  surface: 'trip-tab',      // where you are mounted — see the table
...
| `user-settings`    | reports height | Your `settings.html` (`capabilities.settingsUi`).      |
| `detail-slot`      | reports height | A narrow column in a place/day/reservation panel.      |
| `action-frame`     | fills          | The modal a table action opens.                        |

The contract differs by shape: on a **filling** surface your `trek:resize` height
is ignored — give the document `height:100%` and scroll an inner element; on a
**reporting** surface let content decide the height and never set `100%`. The
design kit handles both from `viewport.fill`. **`insets` is space the host
already keeps clear** (floating controls above, a translucent dock below on the
mobile trip tab) — use it to align your own sticky chrome, never as margin to
add (that doubles the gap).

Inside the mobile shell, `tokens` carries a **second family** alongside the
global one — `--m-ink`, `--m-muted`, `--m-bg`, `--m-card`, `--m-cbr`,
`--m-glass`, `--m-act`, the `--m-st-*` status canon, `--m-safe-top`, … — which is
what the native mobile screens are built from. They are **absent on desktop**,
so branch on presence: `background: var(--m-card, var(--bg-card))`.

Four rules that decide whether it feels native on a phone: inputs **≥ 16px**
(below that iOS zooms on focus and stays zoomed), touch feedback on `:active`
not `:hover`, targets **≥ 44px**, and **no drop shadows** on the translucent
mobile surface. The kit applies all four once `data-form-factor="phone"` is set.
...
- **`window.trek`**: `onContext(cb)` (fires immediately if context already
  ...
  `confirm(opts)` → `Promise<boolean>`, `onEvent(cb)`,
  `session.get/set/remove/clear(…, { scope: 'plugin'|'trip' })` (host-managed
  per-tab JSON state — 32 keys / 64-char keys / 1 KiB values per scope), and
  `geolocation.get()` / `.watch(cb)` (needs the `geolocation:read` permission;
  details below).
