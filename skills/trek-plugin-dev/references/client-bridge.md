...
| Message | Payload |
|---|---|
| `trek:context` | `{ tripId, userId, theme, locale, hostOrigin }` — `userId` is a **string or `null`** (not a number); `theme` is `'light'`/`'dark'` but is **absent on TREK ≤ 3.2.0** — fall back to the OS scheme (see [Making the UI feel native](#making-the-ui-feel-native)) |
...
function resolveTheme(m) {
  ...
  // Older TREK (<= 3.2.0) omits theme — fall back to the OS scheme
...
> **Known host gap (TREK ≤ 3.2.0):** `trek:context` shipped `locale` but **no
> `theme`**, so a naive `m.theme === 'dark' ? …` renders light while TREK is in
> dark mode (and because `locale` still works, the context *looks* fine).
> Current TREK **does** send `theme` — derived from its `.dark` class in
> `PluginFrame.tsx`. Be defensive as above (use `m.theme` when it's a valid
> string, else the OS scheme), and re-resolve on OS `change` events while the
> host gave no theme. When the host sends a real `theme`, it wins.
...
A `hero` widget is a boarding-pass overlay, also not a card — draw it to sit on
the hero bar, not as a panel.
