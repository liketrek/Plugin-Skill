...
export interface PluginContext {
    readonly id: string;
    readonly config: Readonly<Record<string, unknown>>;    // instance-scoped, frozen at activation; an unset field = its manifest `default`
    db: {
    ...
    settings: { get(key: string): Promise<unknown> };   // acting user's own scope:'user' value, else its manifest `default`; undefined if neither / userless
...
| `ctx.settings.get(key)`                                                                                                                          | The **acting user's own** decrypted value for a `scope:'user'` field, or the field's manifest `default` when they never set one. `undefined` for a field with neither, and in a **userless** (`onLoad`/job/scheduler) context — fall back to `ctx.config`. Wired unconditionally (no grant).                                                                                                                                                                                                                                                                                                                                                                                                                                                               | —                                                                            |
